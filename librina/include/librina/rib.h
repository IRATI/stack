...
/// Encodes and Decodes a RIB object to/from bytes)
...
/// Interface to a class that contains a collection of encoders. Given a cdap
/// message, it can encode and decode depending on the object class
class ManagerEncoderInterface
{
 public:
  virtual ~ManagerEncoderInterface()
  {
  }
  ;

  /// Adds an specific encoder into the collection
  /// @param object class of the encoder
  /// @param encoder of the object class
  void addEncoder(const std::string& object_class,
                  rina::EncoderInterface *encoder);

  /// Encodes an object into the object value of a CDAP Message
  /// @param object to encode
  /// @param cdapMessage
  virtual void encode(const void* object, rina::CDAPMessage * cdapMessage) = 0;

  /// Decodes an object contained in the CDAP message object value field
  /// @param CDAPMessage
  /// @throws exception if the byte array is not an encoded in a way that the encoder can recognize, or the
  /// byte array value doesn't correspond to an object of the type "className"
...
class RIBObjectData
{
 public:
  RIBObjectData();
  RIBObjectData(std::string clazz, std::string name, long long instance);
...
class IRIBDaemon
{
 public:
  ...
  virtual std::list<RIBObjectData> getRIBObjectsData() const = 0;
  ...
  virtual void openApplicationConnection(rina::CDAPMessage::AuthTypes auth_mech,
                                         const rina::AuthValue &auth_value,
                                         const std::string &dest_ae_inst,
                                         const std::string &dest_ae_name,
                                         const std::string &dest_ap_inst,
                                         const std::string &dest_ap_name,
                                         const std::string &src_ae_inst,
                                         const std::string &src_ae_name,
                                         const std::string &src_ap_inst,
  ...
  virtual void closeApplicationConnection(
  ...
  /// @param remote_id
  /// @param response_handler
  virtual void remoteCreateObject(
  ...
  /// @param remote_id
  /// @param response_handler
  virtual void remoteDeleteObject(
      const std::string& object_class, const std::string& object_name,
  ...
  /// @param remote_id
  /// @param response_handler
  virtual void remoteWriteObject(
      const std::string& object_class, const std::string& object_name,
      RIBObjectValue& object_value, int scope, const RemoteProcessId& remote_id,
      ICDAPResponseMessageHandler * response_handler) = 0;
  ...
  /// @param remote_id
  /// @param response_handler
  virtual void remoteStartObject(
      const std::string& object_class, const std::string& object_name,
      RIBObjectValue& object_value, int scope, const RemoteProcessId& remote_id,
      ICDAPResponseMessageHandler * response_handler) = 0;
  ...
  /// @param invoke_id
  /// @param remote_id
  virtual void closeApplicationConnectionResponse(
  ...
  /// @param invoke_id
  /// @param remote_id
  virtual void remoteCreateObjectResponse(const std::string& object_class,
  ...
  /// @param invoke_id
  /// @param remote_id
  virtual void remoteDeleteObjectResponse(const std::string& object_class,
                                          const std::string& object_name,
                                          int result,
                                          const std::string result_reason,
                                          int invoke_id,
  ...
  /// @param invoke_id
  /// @param remote_id
  virtual void remoteReadObjectResponse(const std::string& object_class,
                                        const std::string& object_name,
                                        RIBObjectValue& object_value,
                                        int result,
                                        const std::string result_reason,
  ...
  /// @param invoke_id
  /// @param remote_id
  virtual void remoteWriteObjectResponse(const std::string& object_class,
                                         const std::string& object_name,
                                         int result,
                                         const std::string result_reason,
                                         int invoke_id,
  ...
  /// @param invoke_id
  /// @param remote_id
  virtual void remoteStartObjectResponse(const std::string& object_class,
                                         const std::string& object_name,
                                         RIBObjectValue& object_value,
                                         int result,
                                         const std::string result_reason,
  ...
  /// @param invoke_id
  /// @param remote_id
  virtual void remoteStopObjectResponse(const std::string& object_class,
                                        const std::string& object_name,
                                        RIBObjectValue& object_value,
                                        int result,
                                        const std::string result_reason,
...
class RIB : public rina::Lockable
{
 ...
 private:
  ...
  BaseRIBObject* getRIBObject(const std::string& objectClass, long instance,
                              bool check);
...
class BaseRIBObject
{
 public:
  ...
  virtual void remoteCreateObject(void * object_value,
  ...
  virtual void remoteReadObject(int invoke_id,
                                CDAPSessionDescriptor * session_descriptor);
  virtual void remoteCancelReadObject(
      int invoke_id, CDAPSessionDescriptor * cdapSessionDescriptor);
...
class RIBDaemon : public IRIBDaemon
{
 public:
  ...
  ~RIBDaemon();
  void initialize(ManagerEncoderInterface * encoder,
  ...
  void deleteObject(const std::string& objectClass,
                    const std::string& objectName, const void* objectValue,
                    const NotificationPolicy * notificationPolicy);
  ...
  void remoteCreateObject(const std::string& object_class,
                          const std::string& object_name,
                          RIBObjectValue& object_value, int scope,
                          const RemoteProcessId& remote_id,
                          ICDAPResponseMessageHandler * response_handler);
  void remoteDeleteObject(const std::string& object_class,
                          const std::string& object_name, int scope,
                          const RemoteProcessId& remote_id,
                          ICDAPResponseMessageHandler * response_handler);
  void remoteReadObject(const std::string& object_class,
                        const std::string& object_name, int scope,
                        const RemoteProcessId& remote_id,
                        ICDAPResponseMessageHandler * response_handler);
  void remoteWriteObject(const std::string& object_class,
                         const std::string& object_name,
                         RIBObjectValue& object_value, int scope,
                         const RemoteProcessId& remote_id,
                         ICDAPResponseMessageHandler * response_handler);
  void remoteStartObject(const std::string& object_class,
                         const std::string& object_name,
                         RIBObjectValue& object_value, int scope,
                         const RemoteProcessId& remote_id,
                         ICDAPResponseMessageHandler * response_handler);
  void remoteStopObject(const std::string& object_class,
                        const std::string& object_name,
                        RIBObjectValue& object_value, int scope,
                        const RemoteProcessId& remote_id,
                        ICDAPResponseMessageHandler * response_handler);
  void openApplicationConnectionResponse(rina::CDAPMessage::AuthTypes auth_mech,
                                         ...
                                         int invoke_id,
                                         const RemoteProcessId& remote_id);
  void closeApplicationConnectionResponse(int result,
                                          const std::string result_reason,
                                          int invoke_id,
                                          const RemoteProcessId& remote_id);
  void remoteCreateObjectResponse(const std::string& object_class,
                                  const std::string& object_name,
                                  RIBObjectValue& object_value, int result,
                                  const std::string result_reason,
                                  int invoke_id,
                                  const RemoteProcessId& remote_id);
  void remoteDeleteObjectResponse(const std::string& object_class,
                                  const std::string& object_name, int result,
                                  const std::string result_reason,
                                  int invoke_id,
                                  const RemoteProcessId& remote_id);
  void remoteReadObjectResponse(const std::string& object_class,
                                const std::string& object_name,
                                RIBObjectValue& object_value, int result,
                                const std::string result_reason,
                                bool read_incomplete, int invoke_id,
                                const RemoteProcessId& remote_id);
  void remoteWriteObjectResponse(const std::string& object_class,
                                 const std::string& object_name, int result,
                                 const std::string result_reason, int invoke_id,
                                 const RemoteProcessId& remote_id);
  void remoteStartObjectResponse(const std::string& object_class,
                                 const std::string& object_name,
                                 RIBObjectValue& object_value, int result,
                                 const std::string result_reason, int invoke_id,
                                 const RemoteProcessId& remote_id);
  void remoteStopObjectResponse(const std::string& object_class,
                                const std::string& object_name,
                                RIBObjectValue& object_value, int result,
                                const std::string result_reason, int invoke_id,
                                const RemoteProcessId& remote_id);
  ...
  /// message (if any)
  virtual void sendMessageSpecific(
      const RemoteProcessId &remote_proc, const rina::CDAPMessage & cdapMessage,
      ICDAPResponseMessageHandler *cdapMessageHandler) = 0;
  std::list<RIBObjectData> getRIBObjectsData() const;
 protected:
 ...
 private:
  ...
  void sendMessageToProcess(const rina::CDAPMessage & cdapMessage,
                            const RemoteProcessId& remote_id,
                            ICDAPResponseMessageHandler * response_handler);
  RIB *rib_;
  CDAPSessionManagerInterface * cdap_session_manager_;
  ManagerEncoderInterface * encoder_;
  IApplicationConnectionHandler * app_conn_handler_;
};
