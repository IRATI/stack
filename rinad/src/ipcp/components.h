...
class IPCProcessComponent
{
 public:
  ...
  virtual void set_dif_configuration(
      const rina::DIFConfiguration& dif_configuration) = 0;
  ...
  int set_policy_set_param_common(IPCProcess * ipcp, const std::string& path,
                                  const std::string& param_name,
                                  const std::string& param_value);
};

extern "C" {
typedef IPolicySet *(*component_factory_create_t)(IPCProcessComponent * ctx);
    ...
    public rina::IApplicationConnectionHandler
{
 public:
  ...
  /// @param event
  virtual void processEnrollmentRequestEvent(
      rina::EnrollToDIFRequestEvent * event) = 0;
  ...
  /// @param apNamingInfo
  /// @return
  virtual bool isEnrolledTo(
      const std::string& applicationProcessName) const = 0;
};
...
class IFlowAllocator : public IPCProcessComponent
{
 public:
  ...
  virtual void submitAllocateRequest(
      rina::FlowRequestEvent& flowRequestEvent) = 0;
...
class INamespaceManager : public IPCProcessComponent
{
 public:
  ...
  /// @param apNamingInfo
  /// @return
  virtual unsigned int getDFTNextHop(
  ...
  /// @param apNamingInfo
  /// @return
  virtual unsigned short getRegIPCProcessId(
      const rina::ApplicationProcessNamingInformation& apNamingInfo) = 0;
  ...
  virtual bool isValidAddress(unsigned int address,
  ...
  virtual unsigned int getAdressByname(
      const rina::ApplicationProcessNamingInformation& name) = 0;
};
...
class INMinusOneFlowManager
{
 public:
  ...
  virtual unsigned int allocateNMinus1Flow(
      const rina::FlowInformation& flowInformation) = 0;

  /// Process the result of an allocate request event
  /// @param event
  /// @throws IPCException
  virtual void allocateRequestResult(
      const rina::AllocateFlowRequestResultEvent& event) = 0;
  ...
  /// @throws IPCException
  virtual void deallocateFlowResponse(
      const rina::DeallocateFlowResponseEvent& event) = 0;

  /// A flow has been deallocated remotely, process
  /// @param portId
  virtual void flowDeallocatedRemotely(
      const rina::FlowDeallocatedEvent& event) = 0;

  /// Return the N-1 Flow descriptor associated to the flow identified by portId
  /// @param portId
  /// @return the N-1 Flow information
  /// @throws IPCException if no N-1 Flow identified by portId exists
  virtual const rina::FlowInformation& getNMinus1FlowInformation(
      int portId) const = 0;

  /// The IPC Process has been unregistered from or registered to an N-1 DIF
  /// @param evet
  /// @throws IPCException
  virtual void processRegistrationNotification(
      const rina::IPCProcessDIFRegistrationEvent& event) = 0;

  /// True if the DIF name is a supoprting DIF, false otherwise
  /// @param difName
  /// @return
  virtual bool isSupportingDIF(
      const rina::ApplicationProcessNamingInformation& difName) = 0;
...
class BaseIPCPRIBObject : public rina::BaseRIBObject
{
 public:
  ...
  ;
  BaseIPCPRIBObject(IPCProcess* ipc_process, const std::string& object_class,
...
{
 public:
  IPCPRIBDaemon(const rina::RIBSchema *rib_schema);
  ...
  /// @param event
  virtual void processQueryRIBRequestEvent(
      const rina::QueryRIBRequestEvent& event) = 0;
};
...
class IPCProcess
{
 public:
  ...
  virtual void set_operational_state(
      const IPCProcessOperationalState& operational_state) = 0;
  virtual const rina::DIFInformation& get_dif_information() const = 0;
  virtual void set_dif_information(
      const rina::DIFInformation& dif_information) = 0;
  virtual const std::list<rina::Neighbor*> get_neighbors() const = 0;

  virtual std::vector<PsFactory>::iterator
  psFactoryLookup(const std::string& component, const std::string& name) = 0;
  virtual int psFactoryPublish(const PsFactory& factory) = 0;
  virtual int psFactoryUnpublish(const std::string& component,
                                 const std::string& name) = 0;
  virtual IPolicySet * psCreate(const std::string& component,
                                const std::string& name,
                                IPCProcessComponent * context) = 0;
  virtual int psDestroy(const std::string& component, const std::string& name,
...
{
 public:
  SimpleIPCPRIBObject(IPCProcess* ipc_process, const std::string& object_class,
