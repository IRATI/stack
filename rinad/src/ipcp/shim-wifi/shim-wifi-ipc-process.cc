...
# if 0
        ...
        LOG_IPCP_INFO("Initialized Shim Wifi IPC Process of type with name: %s, instance %s, id %hu ",
        		type.c_str(), name_.c_str(), instance_.c_str(), id);
}
...
void ShimWifiIPCProcessImpl::deallocate_flow_response_handler(const rina::DeallocateFlowResponseEvent& event)
{
	resource_allocator_->get_n_minus_one_flow_manager()->deallocateFlowResponse(event);
}

void ShimWifiIPCProcessImpl::flow_deallocated_handler(const rina::FlowDeallocatedEvent& event)
{
...
void ShimWifiIPCProcessImpl::query_rib_handler(const rina::QueryRIBRequestEvent& event)
{
	rib_daemon_->processQueryRIBRequestEvent(event);
	requestPDUFTEDump();
}

void ShimWifiIPCProcessImpl::create_efcp_connection_response_handler(const rina::CreateConnectionResponseEvent& event)
{
	flow_allocator_->processCreateConnectionResponseEvent(event);
}

void ShimWifiIPCProcessImpl::create_efcp_connection_result_handler(const rina::CreateConnectionResultEvent& event)
{
	flow_allocator_->processCreateConnectionResultEvent(event);
}

void ShimWifiIPCProcessImpl::update_efcp_connection_response_handler(const rina::UpdateConnectionResponseEvent& event)
{
	flow_allocator_->processUpdateConnectionResponseEvent(event);
}

void ShimWifiIPCProcessImpl::destroy_efcp_connection_result_handler(const rina::DestroyConnectionResultEvent& event)
{
	if (event.result != 0) {
...
void ShimWifiIPCProcessImpl::dump_ft_response_handler(const rina::DumpFTResponseEvent& event)
{
	if (event.result != 0) {
...
void ShimWifiIPCProcessImpl::set_policy_set_param_handler(const rina::SetPolicySetParamRequestEvent& event)
{
	rina::ScopedLock g(*lock_);
        std::string component, remainder;
...
void ShimWifiIPCProcessImpl::select_policy_set_handler(const rina::SelectPolicySetRequestEvent& event)
{
	rina::ScopedLock g(*lock_);
        bool got_in_userspace;
...
void ShimWifiIPCProcessImpl::select_policy_set_response_handler(const rina::SelectPolicySetResponseEvent& event)
{
	rina::ScopedLock g(*lock_);
	std::map<unsigned int,
...
void ShimWifiIPCProcessImpl::plugin_load_handler(const rina::PluginLoadRequestEvent& event)
{
	rina::ScopedLock g(*lock_);
        int result;
...
void ShimWifiIPCProcessImpl::update_crypto_state_response_handler(const rina::UpdateCryptoStateResponseEvent& event)
{
	security_manager_->process_update_crypto_state_response(event);
}

void ShimWifiIPCProcessImpl::fwd_cdap_msg_handler(const rina::FwdCDAPMsgRequestEvent& event)
{
