...
bool test_flow(rinad::Encoder * encoder)
{
  ...
  // Set
  flow_to_encode.source_naming_info = rina::ApplicationProcessNamingInformation(
      "test", "1");
  flow_to_encode.destination_naming_info =
      rina::ApplicationProcessNamingInformation("test2", "1");
  ...
  connection_policies_to_encode.set_initial_seq_num_policy(
      rina::PolicyConfig("policy1", "23"));
  ...
  // Assert
  if (pflow_to_encode->source_naming_info.processName
      != pflow_decoded->source_naming_info.processName)
    return false;
  if (pflow_to_encode->source_naming_info.processInstance
      != pflow_decoded->source_naming_info.processInstance)
    return false;

  rina::Connection *pconnection_decoded = pflow_decoded->connections.front();
  rina::ConnectionPolicies connection_policies_decoded = pconnection_decoded
      ->getPolicies();
  if (connection_policies_to_encode.get_seq_num_rollover_threshold()
      != connection_policies_decoded.get_seq_num_rollover_threshold())
    return false;
  if (connection_policies_to_encode.get_initial_a_timer()
      != connection_policies_decoded.get_initial_a_timer())
    return false;
  if (connection_policies_to_encode.get_initial_seq_num_policy()
      != connection_policies_decoded.get_initial_seq_num_policy())
    return false;

  if (connection_policies_to_encode.is_dtcp_present()
      != connection_policies_decoded.is_dtcp_present())
    return false;
  else {

    rina::DTCPConfig dtcp_config_decoded = connection_policies_decoded
        .get_dtcp_configuration();
    if (dtcp_config_to_encode.is_rtx_control()
        != dtcp_config_decoded.is_rtx_control())
      return false;
    if (dtcp_config_to_encode.get_rtx_control_config().get_data_rxmsn_max()
        != dtcp_config_decoded.get_rtx_control_config().get_data_rxmsn_max())
      return false;
    if (dtcp_config_to_encode.get_rtx_control_config().get_max_time_to_retry()
        != dtcp_config_decoded.get_rtx_control_config().get_max_time_to_retry())
      return false;
    if (dtcp_config_to_encode.get_rtx_control_config().get_initial_rtx_time()
        != dtcp_config_decoded.get_rtx_control_config().get_initial_rtx_time())
      return false;
    if (dtcp_config_to_encode.is_flow_control()
        != dtcp_config_decoded.is_flow_control())
      return false;
    else {
      rina::DTCPFlowControlConfig flow_config_decoded = dtcp_config_decoded
          .get_flow_control_config();
      if (flow_config_to_encode.get_rcv_buffers_threshold()
          != flow_config_decoded.get_rcv_buffers_threshold())
        return false;
      if (flow_config_to_encode.get_rcv_bytes_percent_threshold()
          != flow_config_decoded.get_rcv_bytes_percent_threshold())
        return false;
      if (flow_config_to_encode.get_rcv_bytes_threshold()
          != flow_config_decoded.get_rcv_bytes_threshold())
        return false;
      if (flow_config_to_encode.get_sent_buffers_threshold()
          != flow_config_decoded.get_sent_buffers_threshold())
        return false;
      if (flow_config_to_encode.get_sent_bytes_percent_threshold()
          != flow_config_decoded.get_sent_bytes_percent_threshold())
        return false;
      if (flow_config_to_encode.get_sent_bytes_threshold()
          != flow_config_decoded.get_sent_bytes_threshold())
        return false;

      if (flow_config_to_encode.is_window_based()
          != flow_config_decoded.is_window_based())
        return false;
      else {
        rina::DTCPWindowBasedFlowControlConfig window_decoded =
            flow_config_decoded.get_window_based_config();
        if (window_to_encode.get_initial_credit()
            != window_decoded.get_initial_credit())
          return false;
        if (window_to_encode.get_maxclosed_window_queue_length()
            != window_decoded.get_maxclosed_window_queue_length())
          return false;
      }

      if (flow_config_to_encode.is_rate_based()
          != flow_config_decoded.is_rate_based())
        return false;
      else {
        rina::DTCPRateBasedFlowControlConfig rate_decoded = flow_config_decoded
            .get_rate_based_config();
        if (rate_to_encode.get_sending_rate()
            != rate_decoded.get_sending_rate())
          return false;
...
bool test_directory_forwarding_table_entry(rinad::Encoder * encoder)
{
  ...
  recovered_obj = (rina::DirectoryForwardingTableEntry*) encoder->decode(
      &cdapMessage);
...
bool test_directory_forwarding_table_entry_list(rinad::Encoder * encoder)
{
  ...
  recovered_obj = (std::list<rina::DirectoryForwardingTableEntry*> *) encoder
      ->decode(&cdapMessage);
...
bool test_enrollment_information_request(rinad::Encoder * encoder)
{
  ...
  recovered_obj = (rinad::EnrollmentInformationRequest *) encoder->decode(
      &cdapMessage);
  ...
  if (request.supporting_difs_.size()
      != recovered_obj->supporting_difs_.size()) {
    return false;
  }
...
bool test_qos_cube(rinad::Encoder * encoder)
{
  ...
  if (cube.peak_bandwidth_duration_
      != recovered_obj->peak_bandwidth_duration_) {
    return false;
  }

  if (cube.peak_sdu_bandwidth_duration_
      != recovered_obj->peak_sdu_bandwidth_duration_) {
    return false;
  }

  if (cube.undetected_bit_error_rate_
      != recovered_obj->undetected_bit_error_rate_) {
    return false;
  }

  if (cube.efcp_policies_.dtcp_present_
      != recovered_obj->efcp_policies_.dtcp_present_) {
    return false;
  }

  if (cube.efcp_policies_.initial_a_timer_
      != recovered_obj->efcp_policies_.initial_a_timer_) {
    return false;
  }
...
bool test_whatevercast_name_list(rinad::Encoder * encoder)
{
  ...
  recovered_obj = (std::list<rina::WhatevercastName*> *) encoder->decode(
      &cdapMessage);
...
bool test_neighbor(rinad::Encoder * encoder)
{
  ...
  nei.name_.processName = "test1.IRATI", nei.name_.processInstance = "1";
  nei.address_ = 25;
  nei.supporting_difs_.push_back(
      rina::ApplicationProcessNamingInformation("test2.IRATI", "1"));
  nei.supporting_difs_.push_back(
      rina::ApplicationProcessNamingInformation("Castefa.i2CAT", "1"));
  ...
  if (nei.name_.processInstance.compare(recovered_obj->name_.processInstance)
      != 0) {
    return false;
  }
...
{
  rinad::FlowStateObject fso = rinad::FlowStateObject(23, 1, 34, 2, true, 123,
                                                      434);
...
bool test_flow_state_object_list(rinad::Encoder * encoder)
{
  ...
  rinad::FlowStateObject fso1 = rinad::FlowStateObject(23, 1, 34, 2, true, 123,
                                                       434);
  rinad::FlowStateObject fso2 = rinad::FlowStateObject(34, 2, 23, 1, true, 223,
                                                       434);
  ...
  recovered_obj = (std::list<rinad::FlowStateObject*> *) encoder->decode(
      &cdapMessage);
