...
{
  rina::PDUFTableGeneratorConfiguration pduftgConfig = dif_configuration
      .get_pduft_generator_configuration();
...
void Graph::init_edges()
{
  ...
  for (flowIt = flow_state_objects_.begin();
    ...
    if (origin->connection_contains_address(dest->address_)
      ...
      edges_.push_back(
          new Edge(origin->address_, (*flowIt)->port_id_, dest->address_,
                   dest->port_id_, 1));
...
FlowStateRIBObjectGroup::FlowStateRIBObjectGroup(
    IPCProcess * ipc_process,
    LinkStatePDUFTGeneratorPolicy * pduft_generator_policy)
    : BaseIPCPRIBObject(
        ipc_process, EncoderConstants::FLOW_STATE_OBJECT_GROUP_RIB_OBJECT_CLASS,
...
void FlowStateRIBObjectGroup::remoteWriteObject(
    ...
    rina::CDAPSessionDescriptor * cdapSessionDescriptor)
{
  ...
  pduft_generator_policy_->writeMessageReceived(
      *objects, cdapSessionDescriptor->get_port_id());
  delete objects;
}
...
FlowStateDatabase::FlowStateDatabase(
    Encoder * encoder, FlowStateRIBObjectGroup * flow_state_rib_object_group,
                                         ...
                                         int neighborPortId)
{
  FlowStateObject * newObject = new FlowStateObject(address, portId,
                                                    neighborAddress,
                                                    neighborPortId, true, 1, 0);

  for (std::list<FlowStateObject *>::iterator it = flow_state_objects_.begin();
      it != flow_state_objects_.end(); ++it) {
...
void LinkStatePDUFTCDAPMessageHandler::readResponse(
    ...
    const rina::DIFConfiguration& dif_configuration)
{
  ...
  if (pduft_generator_config_.get_link_state_routing_configuration()
      .get_routing_algorithm().compare("Dijkstra") != 0) {
  ...
  if (!test_) {
    maximum_age_ =
        pduft_generator_config_.get_link_state_routing_configuration()
            .get_object_maximum_age();
    long delay = 0;

    // Task to compute PDUFT
    delay = pduft_generator_config_.get_link_state_routing_configuration()
        .get_wait_until_pduft_computation();
    ComputePDUFTTimerTask * cttask = new ComputePDUFTTimerTask(this, delay);
    timer_->scheduleTask(cttask, delay);

    // Task to increment age
    delay = pduft_generator_config_.get_link_state_routing_configuration()
        .get_wait_until_age_increment();
    ...
    delay = pduft_generator_config_.get_link_state_routing_configuration()
        .get_wait_until_fsodb_propagation();
...
void LinkStatePDUFTGeneratorPolicy::propagateFSDB() const
{
  ...
  std::list<rina::FlowInformation> nMinusOneFlows = ipc_process_
      ->resource_allocator_->get_n_minus_one_flow_manager()
      ->getAllNMinusOneFlowInformation();

  std::vector<std::list<FlowStateObject *> > groupsToSend = db_
      ->prepareForPropagation(nMinusOneFlows);
...
void LinkStatePDUFTGeneratorPolicy::forwardingTableUpdate()
{
  ...
  std::list<rina::PDUForwardingTableEntry *> pduft = routing_algorithm_
      ->computePDUTForwardingTable(flow_state_objects, source_vertex_);
  try {
                                                        ...
                                                        int portId) const
{
  ...
  try {
    ...
    rib_daemon_->remoteReadObjectResponse(fs_rib_group_->get_class(),
                                          fs_rib_group_->get_name(),
                                          robject_value, 0, "", invoke_id,
                                          false, remote_id);
  } catch (Exception &e) {
    LOG_ERR("Problems encoding and sending CDAP message: %s", e.what());
  }
    ...
    const rina::messages::flowStateObject_t & gpb_fso)
{
  FlowStateObject * fso = new FlowStateObject(gpb_fso.address(),
                                              ...
                                              gpb_fso.neighbor_portid(),
                                              gpb_fso.state(),
                                              gpb_fso.sequence_number(),
                                              gpb_fso.age());
