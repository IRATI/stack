...
#define IPCP_MODULE "routing-ps-multipath"
#include "../../ipcp-logging.h"

#include <librina/timer.h>

#include "ipcp/components.h"
#include "../default/routing-ps.h"
    ...
    /*for (it = graph.vertices_.begin(); it != graph.vertices_.end(); ++it) {
        if ((*it) != source_address) {
            ...
            if (!nextHops.empty()) {
                ...
                entry->qosId = 1;
                entry->cost = 1;
                for (nextHopsIt = nextHops.begin(); nextHopsIt != nextHops.end(); ++nextHopsIt) {
                    entry->nextHopAddresses.push_back(*nextHopsIt);
                    LOG_IPCP_DBG("Added entry to routing table: destination %u, next-hop %u",
                        entry->address, *nextHopsIt);
                }
...
init(IPCProcess * ipc_process, const std::string& plugin_name)
{
	...
	factory.info.name = "multipath";
        factory.info.app_entity = IRoutingComponent::ROUTING_COMPONENT_AE_NAME;
        factory.create = createRoutingComponentPs;
        factory.destroy = destroyRoutingComponentPs;
