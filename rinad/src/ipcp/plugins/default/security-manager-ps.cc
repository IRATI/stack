							   	   	  ...
							   	   	  int mode)
{
	...
	if (mode == rina::AuthSSH2PolicySet::ENCRYPTION) {
		LOG_DBG("Enabling encryption for N-1 port-id: %d", sc->id);
	} else if (mode == rina::AuthSSH2PolicySet::DECRYPTION) {
		LOG_DBG("Enabling decryption for N-1 port-id: %d", sc->id);
	} else {
		LOG_DBG("Enabling encryption and decryption for N-1 port-id: %d", sc->id);
	}
