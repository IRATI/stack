...
{
 public:
  FlowStateObject(unsigned int address, int port_id,
                  unsigned int neighbor_address, int neighbor_port_id,
                  bool state, int sequence_number, int age);
...
{
 public:
  Edge(unsigned int address1, int portV1, unsigned int address2, int portV2,
       int weight);
...
class IRoutingAlgorithm
{
 public:
  ...
  //caller
  virtual std::list<rina::PDUForwardingTableEntry *> computePDUTForwardingTable(
...
class DijkstraAlgorithm : public IRoutingAlgorithm
{
 ...
 private:
  ...
  PredecessorInfo * getNextNode(unsigned int address,
...
class FlowStateDatabase
{
 public:
  ...
  FlowStateDatabase(Encoder * encoder,
  ...
  std::vector<std::list<FlowStateObject*> > prepareForPropagation(
...
class LinkStatePDUFTCDAPMessageHandler :
    public rina::BaseCDAPResponseMessageHandler
{
 public:
  LinkStatePDUFTCDAPMessageHandler(
      LinkStatePDUFTGeneratorPolicy * pduft_generator_policy);
