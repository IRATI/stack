...
void NMinusOneFlowManager::set_dif_configuration(
...
const rina::FlowInformation& NMinusOneFlowManager::getNMinus1FlowInformation(
    int portId) const
{
...
unsigned int NMinusOneFlowManager::allocateNMinus1Flow(
    const rina::FlowInformation& flowInformation)
{
  unsigned int handle = 0;

  try {
    handle = rina::extendedIPCManager->requestFlowAllocationInDIF(
        flowInformation.localAppName, flowInformation.remoteAppName,
        flowInformation.difName, flowInformation.flowSpecification);
  } catch (rina::FlowAllocationException &e) {
    throw Exception(e.what());
  }

  LOG_INFO(
      "Requested the allocation of N-1 flow to application %s-%s through DIF %s",
      flowInformation.remoteAppName.processName.c_str(), flowInformation.remoteAppName.processInstance.c_str(), flowInformation.difName.processName.c_str());

  return handle;
}

void NMinusOneFlowManager::allocateRequestResult(
    const rina::AllocateFlowRequestResultEvent& event)
{
  if (event.portId <= 0) {
    LOG_ERR("Allocation of N-1 flow denied. Error code: %d", event.portId);
    rina::FlowInformation flowInformation = rina::extendedIPCManager
        ->withdrawPendingFlow(event.sequenceNumber);
    Event * flowFailedEvent = new NMinusOneFlowAllocationFailedEvent(
        event.sequenceNumber, flowInformation, "" + event.portId);
    rib_daemon_->deliverEvent(flowFailedEvent);
    return;
  }

  rina::Flow * flow = rina::extendedIPCManager->commitPendingFlow(
      event.sequenceNumber, event.portId, event.difName);
  try {
    std::stringstream ss;
    ...
    rib_daemon_->createObject(
        EncoderConstants::N_MINUS_ONE_FLOW_RIB_OBJECT_CLASS, ss.str(),
        &(flow->getFlowInformation()), 0);
  } catch (Exception &e) {
    LOG_ERR("Problems creating RIB object: %s", e.what());
  }

  Event * flowAllocatedEvent = new NMinusOneFlowAllocatedEvent(
      event.sequenceNumber, flow->getFlowInformation());
  rib_daemon_->deliverEvent(flowAllocatedEvent);
}

void NMinusOneFlowManager::flowAllocationRequested(
    const rina::FlowRequestEvent& event)
{
  if (event.localApplicationName.processName.compare(
    ...
    LOG_ERR(
        "Rejected flow request from %s-%s since this IPC Process is not the intended target of this flow",
        event.remoteApplicationName.processName.c_str(), event.remoteApplicationName.processInstance.c_str());
    try {
  ...
  if (ipc_process_->get_operational_state() != ASSIGNED_TO_DIF) {
    LOG_ERR(
        "Rejecting flow request since the IPC Process is not ASSIGNED to a DIF");
    try {
      rina::extendedIPCManager->allocateFlowResponse(event, -1, true);
    } catch (Exception &e) {
      LOG_ERR("Problems communicating with the IPC Manager: %s", e.what());
  ...
  if (rina::extendedIPCManager->getFlowToRemoteApp(event.remoteApplicationName)
      != 0) {
    LOG_INFO(
        "Rejecting flow request since we already have a flow to the remote IPC Process: %s-%s",
        event.remoteApplicationName.processName.c_str(), event.remoteApplicationName.processInstance.c_str());
    try {
      rina::extendedIPCManager->allocateFlowResponse(event, -1, true);
    } catch (Exception &e) {
  ...
  LOG_INFO(
      "Accepted new flow from IPC Process %s-%s",
      event.remoteApplicationName.processName.c_str(), event.remoteApplicationName.processInstance.c_str());
  try {
    std::stringstream ss;
    ss << EncoderConstants::N_MINUS_ONE_FLOW_SET_RIB_OBJECT_NAME;
...
void NMinusOneFlowManager::flowDeallocatedRemotely(
    const rina::FlowDeallocatedEvent& event)
{
  try {
...
void NMinusOneFlowManager::cleanFlowAndNotify(int portId)
{
  try {
    ...
    rib_daemon_->deleteObject(
        EncoderConstants::N_MINUS_ONE_FLOW_RIB_OBJECT_CLASS, ss.str(), 0, 0);
  } catch (Exception &e) {
    LOG_ERR("Problems deleting object from the RIB: %s", e.what());
  }

  //Notify about the event
  const rina::CDAPSessionInterface * cdapSession = cdap_session_manager_
      ->get_cdap_session(portId);
...
void NMinusOneFlowManager::processRegistrationNotification(
    const rina::IPCProcessDIFRegistrationEvent& event)
{
  if (event.isRegistered()) {
    try {
      rina::extendedIPCManager->appRegistered(event.getIPCProcessName(),
                                              event.getDIFName());
    } catch (Exception &e) {
      LOG_ERR("Problems communicating with the IPC Manager: %s", e.what());
    }
    ...
    try {
      ...
      rib_daemon_->createObject(
          EncoderConstants::DIF_REGISTRATION_RIB_OBJECT_CLASS, ss.str(),
          dif_name, 0);
    } catch (Exception &e) {
      LOG_ERR("Problems creating RIB object: %s", e.what());
      ;
  ...
  try {
    rina::extendedIPCManager->appUnregistered(event.getIPCProcessName(),
                                              event.getDIFName());
  } catch (Exception &e) {
    LOG_ERR("Problems communicating with the IPC Manager: %s", e.what());
  }
  ...
  try {
    ...
    ss << EncoderConstants::SEPARATOR << event.getDIFName().processName;
    rib_daemon_->deleteObject(
        EncoderConstants::DIF_REGISTRATION_RIB_OBJECT_CLASS, ss.str(), 0, 0);
  } catch (Exception &e) {
    LOG_ERR("Problems deleting object from RIB: %s", e.what());
  }
}

bool NMinusOneFlowManager::isSupportingDIF(
    const rina::ApplicationProcessNamingInformation& difName)
{
  ...
  for (unsigned int i = 0; i < registrations.size(); i++) {
    ...
    for (std::list<rina::ApplicationProcessNamingInformation>::iterator it =
        namesList.begin(); it != namesList.end(); ++it) {
...
{
  std::vector<rina::Flow *> flows =
      rina::extendedIPCManager->getAllocatedFlows();
...
void ResourceAllocator::set_dif_configuration(
    const rina::DIFConfiguration& dif_configuration)
{
  if (n_minus_one_flow_manager_) {
...
DIFRegistrationRIBObject::DIFRegistrationRIBObject(
    IPCProcess* ipc_process, const std::string& object_class,
    const std::string& object_name, const std::string* dif_name)
...
DIFRegistrationSetRIBObject::DIFRegistrationSetRIBObject(
...
{
  DIFRegistrationRIBObject * ribObject = new DIFRegistrationRIBObject(
      ipc_process_, objectClass, objectName, (const std::string *) objectValue);
  rib_daemon_->addRIBObject(ribObject);
}

//Class N-1 Flow RIB Object
NMinusOneFlowRIBObject::NMinusOneFlowRIBObject(
    IPCProcess* ipc_process, const std::string& object_class,
    const std::string& object_name, const rina::FlowInformation* flow_info)
    : SimpleSetMemberIPCPRIBObject(ipc_process, object_class, object_name,
                                   flow_info)
{
}
...
std::string NMinusOneFlowRIBObject::get_displayable_value()
{
  const rina::FlowInformation * flow_info =
      (const rina::FlowInformation *) get_value();
  std::stringstream ss;
...
void NMinusOneFlowSetRIBObject::createObject(const std::string& objectClass,
                                             ...
                                             const void* objectValue)
{
  NMinusOneFlowRIBObject * ribObject = new NMinusOneFlowRIBObject(
