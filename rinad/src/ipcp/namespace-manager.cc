...
WhateverCastNameSetRIBObject::WhateverCastNameSetRIBObject(
    ...
    : BaseIPCPRIBObject(
        ipc_process, EncoderConstants::WHATEVERCAST_NAME_SET_RIB_OBJECT_CLASS,
...
void WhateverCastNameSetRIBObject::remoteCreateObject(
    ...
    rina::CDAPSessionDescriptor * session_descriptor)
{
  ...
  try {
    if (object_name.compare(
        EncoderConstants::WHATEVERCAST_NAME_SET_RIB_OBJECT_CLASS) == 0) {
      std::list<rina::WhatevercastName *> * names = (std::list<
          rina::WhatevercastName *> *) object_value;
  ...
  try {
    rib_daemon_->createObject(
        ...
        EncoderConstants::WHATEVERCAST_NAME_SET_RIB_OBJECT_NAME, &namesToCreate,
        0);
  } catch (Exception &e) {
                                                ...
                                                const void* objectValue)
{
  ...
  if (objectClass.compare(
      EncoderConstants::WHATEVERCAST_NAME_SET_RIB_OBJECT_CLASS) == 0) {
    std::list<rina::WhatevercastName *>::const_iterator iterator;
    std::list<rina::WhatevercastName *> * names = (std::list<
        rina::WhatevercastName *> *) objectValue;
...
{
  std::stringstream ss;
  ss << EncoderConstants::WHATEVERCAST_NAME_SET_RIB_OBJECT_NAME
     << EncoderConstants::SEPARATOR;
  ss << name->rule_;
  BaseRIBObject * ribObject = new SimpleSetMemberIPCPRIBObject(
      ipc_process_, EncoderConstants::WHATEVERCAST_NAME_RIB_OBJECT_CLASS,
      ss.str(), name);
  try {
...
DirectoryForwardingTableEntryRIBObject::DirectoryForwardingTableEntryRIBObject(
    IPCProcess * ipc_process, const std::string& object_name,
    rina::DirectoryForwardingTableEntry * entry)
    : SimpleSetMemberIPCPRIBObject(ipc_process,
...
void DirectoryForwardingTableEntryRIBObject::remoteCreateObject(
    void * object_value, const std::string& object_name, int invoke_id,
    rina::CDAPSessionDescriptor * session_descriptor)
{
  rina::DirectoryForwardingTableEntry * entry;
  ...
  if (entry->getKey().compare(ap_name_entry_.getEncodedString()) != 0) {
    LOG_ERR(
        "Keys of the received and existing entries are different, cannot update");
    delete entry;
    return;
  }
  ...
  if (currentEntry->get_address() != entry->get_address()) {
    ...
    rina::NotificationPolicy notificationPolicy = rina::NotificationPolicy(
        cdapSessionIds);
    try {
      rib_daemon_->createObject(EncoderConstants::DFT_ENTRY_RIB_OBJECT_CLASS,
                                object_name, currentEntry, &notificationPolicy);
    } catch (Exception &e) {
      LOG_ERR("Problems creating RIB object: %s", e.what());
    }
  }
...
void DirectoryForwardingTableEntryRIBObject::createObject(
...
void DirectoryForwardingTableEntryRIBObject::remoteDeleteObject(
    int invoke_id, rina::CDAPSessionDescriptor * session_descriptor)
{
  ...
  rina::NotificationPolicy notificationPolicy = rina::NotificationPolicy(
      cdapSessionIds);
  try {
    rib_daemon_->deleteObject(get_class(), get_name(), 0, &notificationPolicy);
  } catch (Exception &e) {
    LOG_ERR("Problems deleting RIB object: %s", e.what());
  }
}

void DirectoryForwardingTableEntryRIBObject::deleteObject(
    const void* objectValue)
{
  (void) objectValue;

  namespace_manager_->removeDFTEntry(ap_name_entry_);
  rib_daemon_->removeRIBObject(get_name());
}
...
DirectoryForwardingTableEntrySetRIBObject::DirectoryForwardingTableEntrySetRIBObject(
    IPCProcess * ipc_process)
    ...
    const std::list<std::string>& namesToDelete)
{
  ...
  for (iterator = namesToDelete.begin(); iterator != namesToDelete.end();
      ++iterator) {
...
void DirectoryForwardingTableEntrySetRIBObject::eventHappened(Event * event)
{
  ...
  ConnectiviyToNeighborLostEvent * conEvent =
  ...
  for (std::list<std::pair<std::string, const void *> >::iterator it = values
    ...
    entry = (rina::DirectoryForwardingTableEntry *) it->second;
    LOG_DBG("Entry pointer: %p", entry);
    if (entry->get_address() == conEvent->neighbor_->get_address()) {
      objectsToDelete.push_back(it->first);
    }
  }
}
    ...
    rina::CDAPSessionDescriptor * session_descriptor)
{
  ...
  try {
    if (object_name.compare(EncoderConstants::DFT_ENTRY_SET_RIB_OBJECT_NAME)
        == 0) {
      std::list<rina::DirectoryForwardingTableEntry *> * entries = (std::list<
          rina::DirectoryForwardingTableEntry *> *) object_value;
      std::list<rina::DirectoryForwardingTableEntry *>::const_iterator iterator;
      for (iterator = entries->begin(); iterator != entries->end();
          ++iterator) {
  ...
  try {
    rib_daemon_->createObject(EncoderConstants::DFT_ENTRY_SET_RIB_OBJECT_CLASS,
                              EncoderConstants::DFT_ENTRY_SET_RIB_OBJECT_NAME,
                              &entriesToCreateOrUpdate, &notificationPolicy);
  } catch (Exception &e) {
    LOG_ERR("Problems creating RIB object: %s", e.what());
  }
}

void DirectoryForwardingTableEntrySetRIBObject::populateEntriesToCreateList(
    rina::DirectoryForwardingTableEntry* entry,
...
void DirectoryForwardingTableEntrySetRIBObject::createObject(
    const std::string& objectClass, const std::string& objectName,
    const void* objectValue)
{
  (void) objectClass;  // Stop compiler barfs
  ...
  rina::DirectoryForwardingTableEntry * currentEntry;
  std::list<rina::DirectoryForwardingTableEntry *> * entries = (std::list<
      rina::DirectoryForwardingTableEntry *> *) objectValue;
  ...
  for (iterator = entries->begin(); iterator != entries->end(); ++iterator) {
    ...
    BaseRIBObject * ribObject = new DirectoryForwardingTableEntryRIBObject(
        ipc_process_, ss.str(), currentEntry);
    try {
      rib_daemon_->addRIBObject(ribObject);
    } catch (Exception &e) {
      LOG_ERR("Problems adding object to the RIB: %s", e.what());
...
void DirectoryForwardingTableEntrySetRIBObject::populateEntriesToDeleteList(
    rina::DirectoryForwardingTableEntry* entry,
    std::list<rina::DirectoryForwardingTableEntry *> * list)
{
  rina::DirectoryForwardingTableEntry * currentEntry;
...
void DirectoryForwardingTableEntrySetRIBObject::deleteObject(
    const void* objectValue)
{
  std::list<rina::DirectoryForwardingTableEntry *>::const_iterator iterator;
  std::list<rina::DirectoryForwardingTableEntry *> * entries = (std::list<
  ...
  for (iterator = entries->begin(); iterator != entries->end(); ++iterator) {
    std::string name;
    if (getObjectName((*iterator)->getKey(), name)) {
      try {
        rib_daemon_->removeRIBObject(name);
      } catch (Exception &e) {
...
bool DirectoryForwardingTableEntrySetRIBObject::getObjectName(
    const std::string& candidateKey, std::string &name)
{
  rina::DirectoryForwardingTableEntry * entry;
...
void NamespaceManager::set_dif_configuration(
...
{
  try {
    BaseIPCPRIBObject * object = new DirectoryForwardingTableEntrySetRIBObject(
        ipc_process_);
...
unsigned int NamespaceManager::getDFTNextHop(
...
void NamespaceManager::removeDFTEntry(
    const rina::ApplicationProcessNamingInformation& apNamingInfo)
{
  rina::DirectoryForwardingTableEntry * entry = dft_.erase(
...
unsigned short NamespaceManager::getRegIPCProcessId(
    const rina::ApplicationProcessNamingInformation& apNamingInfo)
{
...
int NamespaceManager::replyToIPCManagerRegister(
    const rina::ApplicationRegistrationRequestEvent& event, int result)
{
  try {
    ...
    const rina::ApplicationRegistrationRequestEvent& event)
{
  ...
  rina::ApplicationProcessNamingInformation appToRegister = event
  ...
  rina::ApplicationRegistrationInformation * registration =
      new rina::ApplicationRegistrationInformation(
  ...
  registration->ipcProcessId = event.applicationRegistrationInformation
      .ipcProcessId;
  ...
  rina::DirectoryForwardingTableEntry * entry =
      new rina::DirectoryForwardingTableEntry();
  ...
  try {
    rib_daemon_->createObject(EncoderConstants::DFT_ENTRY_SET_RIB_OBJECT_CLASS,
                              EncoderConstants::DFT_ENTRY_SET_RIB_OBJECT_NAME,
                              &entriesToCreate, &notificationPolicy);
  } catch (Exception &e) {
    LOG_ERR("Problems creating RIB object: %s", e.what());
  }
}

int NamespaceManager::replyToIPCManagerUnregister(
    const rina::ApplicationUnregistrationRequestEvent& event, int result)
{
  try {
    ...
    const rina::ApplicationUnregistrationRequestEvent& event)
{
  ...
  rina::ApplicationRegistrationInformation * unregisteredApp = registrations_
      .erase(event.applicationName.getEncodedString());
  ...
  if (result == -1) {
    registrations_.put(event.applicationName.getEncodedString(),
                       unregisteredApp);
    return;
  }
  ...
  try {
    ...
    ss << EncoderConstants::SEPARATOR
       << unregisteredApp->appName.getEncodedString();
    rib_daemon_->deleteObject(EncoderConstants::DFT_ENTRY_RIB_OBJECT_CLASS,
...
unsigned int NamespaceManager::getIPCProcessAddress(
    const std::string& process_name, const std::string& process_instance,
...
unsigned int NamespaceManager::getAddressPrefix(
...
bool NamespaceManager::isValidAddress(unsigned int address,
                                      const std::string& ipcp_name,
                                      const std::string& ipcp_instance)
{
  ...
  rina::AddressingConfiguration configuration = ipc_process_
      ->get_dif_information().dif_configuration_.nsm_configuration_
      .addressing_configuration_;
  unsigned int knownAddress = getIPCProcessAddress(ipcp_name, ipcp_instance,
                                                   configuration);
  if (knownAddress != 0) {
                                               ...
                                               const std::string& ipcp_instance)
{
  rina::AddressingConfiguration configuration = ipc_process_
      ->get_dif_information().dif_configuration_.nsm_configuration_
      .addressing_configuration_;
  unsigned int candidateAddress = getIPCProcessAddress(ipcp_name, ipcp_instance,
                                                       configuration);
  ...
  while (candidateAddress
      < prefix + rina::AddressPrefixConfiguration::MAX_ADDRESSES_PER_PREFIX) {
...
unsigned int NamespaceManager::getAdressByname(
