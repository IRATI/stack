...
void FlowRIBObject::remoteDeleteObject(int invoke_id,
		...
		const rina::DIFConfiguration& dif_configuration)
{
	...
	std::stringstream ss;
	const std::list<rina::QoSCube*>& cubes =
			dif_configuration.efcp_configuration_.qos_cubes_;
		...
		const std::string& object_name, int invoke_id, int underlyingPortId)
{
	...
	if (address == myAddress) {
		...
		flowAllocatorInstance->createFlowRequestMessageReceived(flow,
				object_name, invoke_id, underlyingPortId);
		return;
	}
	...
	try {
		...
		allocate_response_message_handle_ =
				rina::extendedIPCManager->allocateFlowRequestArrived(
						flow_->destination_naming_info,
						flow_->source_naming_info, flow_->flow_specification,
						port_id_);
		LOG_DBG(
		...
		const rina::AllocateFlowResponseEvent& event)
{
	...
	try {
		...
		rib_daemon_->remoteCreateObjectResponse(
				EncoderConstants::FLOW_RIB_OBJECT_CLASS, object_name_,
				robject_value, -1, "Application rejected the flow", invoke_id_,
				remote_id);
	} catch (Exception &e) {
		LOG_ERR("Problems requesting RIB Daemon to send CDAP Message: %s",
				e.what());
...
void FlowAllocatorInstance::createResponse(int result,
...
{
	return &(ipc_process_->get_dif_information().dif_configuration_.efcp_configuration_.data_transfer_constants_);
}

std::string DataTransferConstantsRIBObject::get_displayable_value()
{
	rina::DataTransferConstants dtc =
			ipc_process_->get_dif_information().dif_configuration_.efcp_configuration_.data_transfer_constants_;
	return dtc.toString();
}
