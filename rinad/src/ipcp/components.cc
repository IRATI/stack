...
std::string Flow::toString()
{
  ...
  ss << "* Is this IPC Process the requestor of the flow? " << source
     << std::endl;
  ss << "* Max create flow retries: " << max_create_flow_retries << std::endl;
  ...
  ss << "* Destination AP Naming Info: " << destination_naming_info.toString();
  ...
  if (connections.size() > 0) {
    ...
    for (std::list<rina::Connection*>::const_iterator iterator = connections
        .begin(), end = connections.end(); iterator != end; ++iterator) {
      ss << "Src CEP-id " << (*iterator)->getSourceCepId() << "; Dest CEP-id "
         << (*iterator)->getDestCepId() << "; Qos-id "
...
// Class BaseRIBObject
BaseIPCPRIBObject::BaseIPCPRIBObject(IPCProcess * ipc_process,
                                     const std::string& object_class,
                                     long object_instance,
                                     const std::string& object_name)
    : rina::BaseRIBObject(ipc_process->rib_daemon_, object_class,
                          object_instance, object_name)
{
  ipc_process_ = ipc_process;
  if (ipc_process) {
    rib_daemon_ = ipc_process->rib_daemon_;
    //FIXME encoder removed
...
SimpleIPCPRIBObject::SimpleIPCPRIBObject(IPCProcess* ipc_process,
                                         const std::string& object_class,
                                         const std::string& object_name,
...
void SimpleIPCPRIBObject::createObject(const std::string& objectClass,
...
SimpleSetIPCPRIBObject::SimpleSetIPCPRIBObject(
...
void SimpleSetIPCPRIBObject::createObject(const std::string& objectClass,
                                          const std::string& objectName,
                                          const void* objectValue)
{
...
SimpleSetMemberIPCPRIBObject::SimpleSetMemberIPCPRIBObject(
    IPCProcess* ipc_process, const std::string& object_class,
...
IPCPRIBDaemon::IPCPRIBDaemon(const rina::RIBSchema *rib_schema)
...
int IPCProcessComponent::set_policy_set_param_common(
    IPCProcess * ipcp, const std::string& path, const std::string& param_name,
