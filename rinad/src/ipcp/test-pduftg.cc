...
class FakeRIBDaemon : public rinad::IPCPRIBDaemon
{
 public:
  ...
  void sendMessageSpecific(
      bool useAddress, const rina::CDAPMessage & cdapMessage, int sessionId,
  ...
  void sendMessageToAddress(
      const rina::CDAPMessage & cdapMessage, int sessionId,
      unsigned int address,
      rina::ICDAPResponseMessageHandler * cdapMessageHandler)
  {
    if (!cdapMessageHandler) {
  ...
  void createObject(const std::string& objectClass,
                    ...
                    const rina::NotificationPolicy * notificationPolicy)
  {
  ...
  void remoteCreateObject(const std::string& object_class,
                          ...
                          rina::RIBObjectValue& object_value, int scope,
                          const rina::RemoteProcessId& remote_id,
                          rina::ICDAPResponseMessageHandler * response_handler)
  {
    (void) object_class;
  ...
  void closeApplicationConnectionResponse(
  ...
  void remoteReadObjectResponse(const std::string& object_class,
                                const std::string& object_name,
                                rina::RIBObjectValue& object_value, int result,
                                const std::string result_reason,
...
class FakeNamespaceManager : public rinad::INamespaceManager
{
 public:
  ...
  unsigned int getDFTNextHop(
      const rina::ApplicationProcessNamingInformation& apNamingInfo)
  {
    (void) apNamingInfo;
    return 0;
  }
  unsigned short getRegIPCProcessId(
      const rina::ApplicationProcessNamingInformation& apNamingInfo)
  {
    (void) apNamingInfo;
    return 0;
...
class FakeIPCProcess : public rinad::IPCProcess
{
 public:
  ...
  void set_operational_state(
      const rinad::IPCProcessOperationalState& operational_state)
  {
...
int addObjectToGroup_AddObjectCheckModified_True()
{
  ...
  rinad::FlowStateRIBObjectGroup group = rinad::FlowStateRIBObjectGroup(
      &ipcProcess, 0);
  rinad::FlowStateDatabase fsdb = rinad::FlowStateDatabase(
      ipcProcess.encoder_, &group, 0, ipcProcess.rib_daemon_, 0);
...
int incrementAge_AddObjectCheckModified_False()
{
  ...
  rinad::FlowStateDatabase fsdb = rinad::FlowStateDatabase(
      ipcProcess.encoder_, &group, 0, ipcProcess.rib_daemon_, &max_age);
...
int getPDUTForwardingTable_NoFSO_size0()
{
  ...
  rinad::IRoutingAlgorithm * routingAlgorithm = new rinad::DijkstraAlgorithm();
  std::list<rinad::FlowStateObject *> fsos;

  std::list<rina::PDUForwardingTableEntry *> pduft = routingAlgorithm
      ->computePDUTForwardingTable(fsos, 1);
...
int getPDUTForwardingTable_LinearGraphNumberOfEntries_2()
{
  ...
  std::list<rina::PDUForwardingTableEntry *> pduft = routingAlgorithm
      ->computePDUTForwardingTable(objects, 1);
...
int flowdeAllocated_DeAllocateFlow_True()
{
  ...
  difConfiguration.pduft_generator_configuration_
      .link_state_routing_configuration_.set_routing_algorithm("Dijkstra");
  ...
  flowInfo.portId = 3;
  rinad::NMinusOneFlowAllocatedEvent event = rinad::NMinusOneFlowAllocatedEvent(
      4, flowInfo);
  ...
  rinad::NMinusOneFlowDeallocatedEvent event2 =
      rinad::NMinusOneFlowDeallocatedEvent(3, 0);
