...
IPCProcessImpl::~IPCProcessImpl()
{
  ...
  if (flow_allocator_) {
    psDestroy("flow-allocator", flow_allocator_->selected_ps_name,
              flow_allocator_->ps);
  ...
  if (security_manager_) {
    psDestroy("security-manager", security_manager_->selected_ps_name,
  ...
  for (std::map<std::string, void *>::iterator it = plugins_handles.begin();
...
void IPCProcessImpl::set_operational_state(
...
void IPCProcessImpl::set_dif_information(
    const rina::DIFInformation& dif_information)
{
  rina::AccessGuard g(*lock_);
...
void IPCProcessImpl::processAssignToDIFRequestEvent(
    const rina::AssignToDIFRequestEvent& event)
{
  ...
  if (state != INITIALIZED) {
    ...
    LOG_ERR(
        "Got a DIF assignment request while not in INITIALIZED state. Current state is: %d",
        state);
  ...
  try {
    unsigned int handle = rina::kernelIPCProcess->assignToDIF(
        event.difInformation);
    pending_events_.insert(
        std::pair<unsigned int, rina::AssignToDIFRequestEvent>(handle, event));
    state = ASSIGN_TO_DIF_IN_PROCESS;
  } catch (Exception &e) {
    LOG_ERR("Problems sending DIF Assignment request to the kernel: %s",
            e.what());
    rina::extendedIPCManager->assignToDIFResponse(event, -1);
  }
}

void IPCProcessImpl::processAssignToDIFResponseEvent(
    const rina::AssignToDIFResponseEvent& event)
{
  ...
  if (state == ASSIGNED_TO_DIF) {
    LOG_INFO(
        "Got reply from the Kernel components regarding DIF assignment: %d",
        event.result);
    return;
  }
  ...
  if (state != ASSIGN_TO_DIF_IN_PROCESS) {
    LOG_ERR(
        "Got a DIF assignment response while not in ASSIGN_TO_DIF_IN_PROCESS state. State is %d ",
        state);
    return;
  }
  ...
  if (it == pending_events_.end()) {
    LOG_ERR(
        "Couldn't find an Assign to DIF request event associated to the handle %u",
  ...
  if (event.result != 0) {
    LOG_ERR(
        "The kernel couldn't successfully process the Assign to DIF Request: %d",
        event.result);
  ...
  try {
    ...
    namespace_manager_->set_dif_configuration(
        dif_information_.dif_configuration_);
    security_manager_->set_dif_configuration(
        dif_information_.dif_configuration_);
    flow_allocator_->set_dif_configuration(dif_information_.dif_configuration_);
    enrollment_task_->set_dif_configuration(
        dif_information_.dif_configuration_);
  } catch (Exception &e) {
    state = INITIALIZED;
...
void IPCProcessImpl::requestPDUFTEDump()
{
  try {
    ...
    LOG_WARN("Error requesting IPC Process to Dump PDU Forwarding Table: %s",
             e.what());
  }
}
    ...
    const rina::SetPolicySetParamRequestEvent& event)
{
  ...
  if (component == "security-manager") {
    result = security_manager_->set_policy_set_param(remainder, event.name,
                                                     event.value);
  } else if (component == "enrollment") {
    result = enrollment_task_->set_policy_set_param(remainder, event.name,
                                                    event.value);
  } else if (component == "flow-allocator") {
    result = flow_allocator_->set_policy_set_param(remainder, event.name,
                                                   event.value);
  } else if (component == "namespace-manager") {
    result = namespace_manager_->set_policy_set_param(remainder, event.name,
                                                      event.value);
  } else if (component == "resource-allocator") {
    result = resource_allocator_->set_policy_set_param(remainder, event.name,
                                                       event.value);
  } else if (component == "rib-daemon") {
    result = rib_daemon_->set_policy_set_param(remainder, event.name,
                                               event.value);
  } else {
  ...
  if (got_in_userspace) {
    ...
    rina::extendedIPCManager->setPolicySetParamResponse(event, result);
    return;
  }

  try {
    unsigned int handle = rina::kernelIPCProcess->setPolicySetParam(
        event.path, event.name, event.value);
    pending_set_policy_set_param_events.insert(
        std::pair<unsigned int, rina::SetPolicySetParamRequestEvent>(handle,
                                                                     event));
  } catch (Exception &e) {
...
{
  rina::AccessGuard g(*lock_);
  std::map<unsigned int, rina::SetPolicySetParamRequestEvent>::iterator it;
  ...
  if (event.result != 0) {
    ...
    "set-policy-set-param Request: %d",
            event.result);
    ...
    const rina::SelectPolicySetRequestEvent& event)
{
  ...
  // or should be forwarded to kernelspace
  if (component == "security-manager") {
    result = security_manager_->select_policy_set(remainder, event.name);
  } else if (component == "enrollment") {
    result = enrollment_task_->select_policy_set(remainder, event.name);
  } else if (component == "flow-allocator") {
    result = flow_allocator_->select_policy_set(remainder, event.name);
  } else if (component == "namespace-manager") {
    result = namespace_manager_->select_policy_set(remainder, event.name);
  } else if (component == "resource-allocator") {
    result = resource_allocator_->select_policy_set(remainder, event.name);
  } else if (component == "rib-daemon") {
    result = rib_daemon_->select_policy_set(remainder, event.name);
  } else {
    got_in_userspace = false;
  }
  ...
  if (got_in_userspace) {
    // Event managed without going through kernelspace. Notify
    // the IPC Manager about the result
    rina::extendedIPCManager->selectPolicySetResponse(event, result);
    return;
  }

  // Forward the request to the kernel
  try {
    unsigned int handle = rina::kernelIPCProcess->selectPolicySet(event.path,
                                                                  event.name);
    pending_select_policy_set_events.insert(
        std::pair<unsigned int, rina::SelectPolicySetRequestEvent>(handle,
                                                                   event));
  } catch (Exception &e) {
...
{
  rina::AccessGuard g(*lock_);
  std::map<unsigned int, rina::SelectPolicySetRequestEvent>::iterator it;
...
int IPCProcessImpl::plugin_unload(const std::string& plugin_name)
{
  ...
  for (std::vector<PsFactory>::iterator it = components_factories.begin();
  ...
  for (unsigned int i = 0; i < unpublish_list.size(); i++) {
    psFactoryUnpublish(unpublish_list[i]->component, unpublish_list[i]->name);
  }
...
std::vector<PsFactory>::iterator IPCProcessImpl::psFactoryLookup(
...
int IPCProcessImpl::psFactoryPublish(const PsFactory& factory)
{
  ...
  if (psFactoryLookup(factory.component, factory.name)
    ...
    "published",
            factory.name.c_str(), factory.component.c_str());
    return -1;
  }
  ...
  LOG_INFO(
      "Pluggable component '%s'/'%s' [%s] published",
      factory.component.c_str(), factory.name.c_str(), factory.plugin_name.c_str());
                                       ...
                                       const std::string& name)
{
  ...
  if (fi == components_factories.end()) {
    ...
    "published",
            name.c_str(), component.c_str());
    return -1;
  }

  LOG_INFO("Pluggable component '%s'/'%s' [%s] unpublished",
           fi->component.c_str(), fi->name.c_str(), fi->plugin_name.c_str());
...
IPolicySet *
IPCProcessImpl::psCreate(const std::string& component, const std::string& name,
...
static void ipc_process_dif_registration_notification_handler(
    rina::IPCEvent *e, EventLoopData *opaque)
{
...
static void assign_to_dif_request_event_handler(rina::IPCEvent *e,
...
static void allocate_flow_request_result_event_handler(rina::IPCEvent *e,
                                                       EventLoopData *opaque)
{
...
static void flow_allocation_requested_event_handler(rina::IPCEvent *e,
                                                    EventLoopData *opaque)
{
...
static void deallocate_flow_response_event_handler(rina::IPCEvent *e,
                                                   EventLoopData *opaque)
{
...
static void flow_deallocated_event_handler(rina::IPCEvent *e,
                                           EventLoopData *opaque)
{
...
static void enroll_to_dif_request_event_handler(rina::IPCEvent *e,
                                                EventLoopData *opaque)
{
...
static void ipc_process_query_rib_handler(rina::IPCEvent *e,
                                          EventLoopData *opaque)
{
...
static void application_registration_request_event_handler(
    rina::IPCEvent *e, EventLoopData *opaque)
{
    ...
    rina::IPCEvent *e, EventLoopData *opaque)
{
  ...
  ipcp->namespace_manager_->processApplicationUnregistrationRequestEvent(
      *event);
}

static void ipc_process_create_connection_response_handler(
    rina::IPCEvent *e, EventLoopData *opaque)
{
...
static void allocate_flow_response_event_handler(rina::IPCEvent *e,
                                                 EventLoopData *opaque)
{
...
static void ipc_process_create_connection_result_handler(rina::IPCEvent *e,
                                                         EventLoopData *opaque)
{
...
static void ipc_process_update_connection_response_handler(
    rina::IPCEvent *e, EventLoopData *opaque)
{
...
static void flow_deallocation_requested_event_handler(rina::IPCEvent *e,
                                                      EventLoopData *opaque)
{
...
static void ipc_process_destroy_connection_result_handler(rina::IPCEvent *e,
                                                          EventLoopData *opaque)
{
...
static void ipc_process_dump_ft_response_handler(rina::IPCEvent *e,
                                                 EventLoopData *opaque)
{
...
static void ipc_process_set_policy_set_param_handler(rina::IPCEvent *e,
                                                     EventLoopData *opaque)
...
static void ipc_process_set_policy_set_param_response_handler(
    rina::IPCEvent *e, EventLoopData *opaque)
...
static void ipc_process_select_policy_set_handler(rina::IPCEvent *e,
                                                  EventLoopData *opaque)
...
static void ipc_process_select_policy_set_response_handler(
    rina::IPCEvent *e, EventLoopData *opaque)
...
static void ipc_process_plugin_load_handler(rina::IPCEvent *e,
                                            EventLoopData *opaque)
...
static void ipc_process_default_handler(rina::IPCEvent *e,
                                        EventLoopData *opaque)
{
  (void) opaque;
...
void register_handlers_all(EventLoop& loop)
{
  ...
  loop.register_event(rina::NEIGHBORS_MODIFIED_NOTIFICATION_EVENT,
                      ipc_process_default_handler);
  loop.register_event(rina::GET_DIF_PROPERTIES, ipc_process_default_handler);
  loop.register_event(rina::GET_DIF_PROPERTIES_RESPONSE_EVENT,
                      ipc_process_default_handler);
  loop.register_event(rina::OS_PROCESS_FINALIZED, ipc_process_default_handler);
  ...
  loop.register_event(rina::IPC_PROCESS_DAEMON_INITIALIZED_EVENT,
                      ipc_process_default_handler);
  loop.register_event(rina::TIMER_EXPIRED_EVENT, ipc_process_default_handler);
