...
void ApplicationBuilder::configure(int argc, char** argv)
{
    ...
    while((c = getopt(argc, argv, "lLxXrRfFhc:s:")) != -1) {
        ...
        case 'c':
            ...
            break;
        case 's':
            data_size = std::min<uint>(Application::max_buffer_size,
...
void ApplicationBuilder::runApplication()
{
    if(server) {
        ...
        Client s(client_name, client_instance, server_name, server_instance,
                 time_flow_creation, echo_times, client_app_reg, data_size, debug_mes);
        s.run();
    }
}
