...
rina::cdap_rib::res_info_t* IPCPObj::remoteRead(
    const std::string& name, rina::cdap_rib::SerializedObject &obj_reply)
{
    ...
    rina::cdap_rib::SerializedObject &obj_reply)
{
  ...
  if (clas == "IPCProcess") {
    ribd_->addRIBObject(new IPCPObj(name, InstanceGenerator->next(), obj_req));
    res->result_ = 1;
    (void) obj_reply;
  } else {
    LOG_ERR(
        "Create object %s is not implemented as a child of OSApplicationProcess object",
        name.c_str());
    res->result_ = -1;
  }
  return res;
