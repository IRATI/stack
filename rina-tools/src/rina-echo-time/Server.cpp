...
Server::Server(const string& app_name_, const string& app_instance_,
               bool debug_mes_): Application(app_name_, app_instance_), debug_mes(debug_mes_)
{
}
    ...
    for(;;) {
        ...
        case FLOW_ALLOCATION_REQUESTED_EVENT: {
            Flow* flow = ipcManager->allocateFlowResponse(
                *reinterpret_cast<FlowRequestEvent*>(event), 0, true);
...
void Server::runFlow(Flow* flow)
{
    ...
    try {
        for(;;) {
            int bytesreaded = flow->readSDU(buffer, max_buffer_size);
