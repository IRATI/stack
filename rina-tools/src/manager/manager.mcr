...
/* mcr */
void printHex(char* buf, int length, int offset, int width) {
  std::stringstream string_stream;
  for (int index=0;index < width;index++) {
    if (index+offset < length) {
      printf("%02x ", buf[index+offset]);
      if (isalnum(buf[index+offset]) || ispunct(buf[index+offset]) || buf[index+offset]==' ' ) {
        string_stream << buf[index+offset];
      } else {
        string_stream << ' ';        
      }
    } else {
