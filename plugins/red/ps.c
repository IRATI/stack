...
static void red_max_q_policy_tx(struct rmt_ps *      ps,
                                     struct pdu *         pdu,
                                     struct rmt_n1_port * port)
{ printk("%s: called()\n", __func__); }

static void red_max_q_policy_rx(struct rmt_ps *      ps,
                                     struct sdu *         sdu,
                                     struct rmt_n1_port * port)
{ printk("%s: called()\n", __func__); }

static void red_rmt_q_monitor_policy_tx(struct rmt_ps *      ps,
                                             struct pdu *         pdu,
                                             struct rmt_n1_port * port)
{ printk("%s: called()\n", __func__); }

static void red_rmt_q_monitor_policy_rx(struct rmt_ps *      ps,
                                             struct sdu *         sdu,
                                             struct rmt_n1_port * port)
{ printk("%s: called()\n", __func__); }
...
rmt_ps_red_create(struct rina_component * component)
{
        ...
        ps->dm = rmt;
        ps->priv = NULL;
        ps->max_q_policy_tx = red_max_q_policy_tx;
        ps->max_q_policy_rx = red_max_q_policy_rx;
        ps->rmt_q_monitor_policy_tx = red_rmt_q_monitor_policy_tx;
        ps->rmt_q_monitor_policy_rx = red_rmt_q_monitor_policy_rx;
...
#define RINA_RMT_REDPCC_NAME   "red-pcc"

static int __init mod_init(void)
{
        int ret;

        strcpy(factory.name, RINA_RMT_REDPCC_NAME);
        ...
        LOG_INFO("RMT RED policy set loaded successfully");
...
{
        int ret = rmt_ps_unpublish(RINA_RMT_REDPCC_NAME);
        ...
        LOG_INFO("RMT RED policy set unloaded successfully");
}

module_init(mod_init);
module_exit(mod_exit);

MODULE_DESCRIPTION("RMT RED policy set");
