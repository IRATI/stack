                                             ...
                                             struct rnl_ipcm_alloc_flow_req_msg_attrs * msg_attrs)
{
	...
	LOG_DBG("rnl_parse_ipcm_alloc_flow_req_msg;\n"
		"Policies @: %p - %p - %p - %p - %p\n"
		"IAFRM_ATTR_SOURCE_APP_NAME: %d\n"
		"IAFRM_ATTR_DEST_APP_NAME: %d\n"
		"IAFRM_ATTR_FLOW_SPEC: %d\n"
		"IAFRM_ATTR_PORT_ID: %d\n"
		"IAFRM_ATTR_DIF_NAME: %d\n",
		&attr_policy[IAFRM_ATTR_SOURCE_APP_NAME],
		&attr_policy[IAFRM_ATTR_DEST_APP_NAME],
		&attr_policy[IAFRM_ATTR_FLOW_SPEC],
		&attr_policy[IAFRM_ATTR_PORT_ID],
		&attr_policy[IAFRM_ATTR_DIF_NAME],
	...
	LOG_DBG("calling parse_app_name_info with %p and %p\n", 
		info->attrs[IAFRM_ATTR_SOURCE_APP_NAME],
		msg_attrs->source);

        if (parse_app_name_info(info->attrs[IAFRM_ATTR_SOURCE_APP_NAME],
                                msg_attrs->source) < 0){
		LOG_DBG("Fail app_name_info\n");
                goto format_fail;
	}
                  ...
                  struct rnl_msg        * msg)
{
	...
	LOG_DBG("[LDBG] (msg->rina_hdr)->src_ipc_id is %d", (msg->rina_hdr)->src_ipc_id);
	LOG_DBG("[LDBG] (msg->rina_hdr)->dst_ipc_id is %d", (msg->rina_hdr)->dst_ipc_id);
