...
static struct nla_policy attr_policy[IAFRA_ATTR_MAX + 1] = {
...
static struct nla_policy ifdn_policy[IFDN_ATTR_MAX + 1] = {
        [IFDN_ATTR_PORT_ID] = {.type = NLA_U32, .len = 0};
        [IFDN_ATTR_CODE]    = {.type = NLA_U32, .len = 0};
};

static struct nla_policy iccrq_policy[ICCRQ_ATTR_MAX + 1] = {
        [ICCRQ_ATTR_PORT_ID]     = {.type = NLA_U32, .len = 0};
        [ICCRQ_ATTR_SOURCE_ADDR] = {.type = NLA_U32, .len = 0};
        [ICCRQ_ATTR_DEST_ADDR]   = {.type = NLA_U32, .len = 0};
        [ICCRQ_ATTR_QOS_ID]      = {.type = NLA_U32, .len = 0};
        [ICCRQ_ATTR_POLICIES]    = {.type = NLA_U32, .len = 0};
};

#define DECL_NL_OP(X, Y) {                      \
                        .cmd    = X,            \
                        .flags  = 0,            \
                        .policy = Y,            \
                        .doit   = dispatcher,   \
