...
static int process_dt_spdu(struct rmt *       rmt,
                          port_id_t          port_id,
                          struct sdu *       sdu,
                          struct pdu *       pdu)
{
...
static int receive_worker(void * o)
{
        ...
        hash_for_each_safe(tmp->ingress.queues->queues,
                ...
                case PDU_TYPE_MGMT:
                        if (!pci_destination(pdu_pci_get_ro(pdu))) { 
                                ...
                                process_dt_spdu(tmp, port_id, sdu, pdu);
                        break;
                ...
                case PDU_TYPE_DT:
                         ...
                         */
                        process_dt_spdu(tmp, port_id, sdu, pdu);
                        LOG_DBG("Finishing process_dt_spdu");
                        break;
        ...
        /* (FUTURE) for-each list in pdus_dt call process_dt_spdus(pdus_dt) */
        ...
        while (!nothing_to_do) {
                ...
                hash_for_each_safe(rmt->ingress.queues->queues,
                        ...
                        case PDU_TYPE_DT:
                                 ...
                                 * don't process it now ...
                                 *
                                 * process_dt_spdu(rmt, port_id, sdu);
                                 */
                                break;
                        default:
                                LOG_ERR("Unknown PDU type %d", pdu_type);
