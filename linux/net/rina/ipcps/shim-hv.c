                      ...
                      uint8_t *msg, int len)
{
        ...
        LOG_DBG("%s: vmpi_write_kernel(0, %d) --> %d", __func__, (int)len, (int)ret);
                  ...
                  struct sdu *sdu)
{
        ...
        if (likely(n == iov.iov_len))
                ret = 0;
        LOG_DBG("%s: vmpi_write_kernel(%u, %d) --> %d", __func__, ch,
...
shim_hv_factory_init(struct ipcp_factory_data *factory_data)
{
        ...
        LOG_DBG("%s initialized", SHIM_HV_NAME);
...
shim_hv_factory_fini(struct ipcp_factory_data *data)
{
        ...
        LOG_DBG("%s uninitialized", SHIM_HV_NAME);
